...
eff <- 0.6  # 60 % due to mixture of hydrogen/e-fuels and transport
flh <- 5000  # h
...
ggsave(paste0(path.output, "FIG01_electrolysis_projects.svg"),
       bg = "white", width = 18, height = 12, units = "cm")
...
ggsave(paste0(path.output, "FIG02_illustration.svg"),
       width = 18, height = 12, units = "cm", bg = "white")
...
### Electrolysis CAPEX and learning rate

```{r, message = FALSE}
# Electrolysis CAPEX and learning rate
file.capex.lr <- paste0(path.input, "Electrolyser_Data_Complete_Efuels_JE_FU_AO.xlsx")
data.capex <- read_excel(file.capex.lr, sheet = "Data", col_names = TRUE) %>% 
  select(Scenario_Year, Temp, Cost_eu_KW, Efficiency_LHV, Method, Bracket) %>% 
  filter(Bracket == 2020,
         Method != "SOEC",
         !is.na(Cost_eu_KW))
data.learning <- read_excel(file.capex.lr, sheet = "Learning_rates", col_names = TRUE) %>% 
  select(Scenario_Year, Learning_rate, Method) %>% 
  filter(Method != "SOEC",
         !is.na(Learning_rate))
```
...
data.targets <- tribble(
  ...
  "Global", 2030, "GW", 154,  # Cumulative announcements
...
ggsave(paste0(path.output, "FIG03_parameter_distributions.svg"),
       width = 18, height = 14, units = "cm", bg = "white")
```
...
ggsave(paste0(path.output, "FIG04_probabilistic_feasibility_space.svg"),
       width = 18, height = 9, units = "cm", bg = "white")
```
...
techs <- unique(data.unconventional$tech)
for (i in techs) {
  # Filter technology
...
# Exclude COVID-19 vaccinations
param.unconventional.fit <- param.unconventional.fit %>% 
...
ggsave(paste0(path.output, "FIG05_unconventional_growth.svg"),
       width = 18, height = 13, units = "cm", bg = "white")
```
...
file.innopaths <- paste0(path.input, "innopaths_final_energy.csv")
...
ggsave(paste0(path.output, "FIG06_conventional_vs_unconventional.png"),
       width = 18, height = 12, units = "cm", bg = "white")
ggsave(paste0(path.output, "FIG06_conventional_vs_unconventional.svg"),
...
data.stat.conv <- data.results %>% 
  ...
  drop_na(forecast) %>%
  filter(year == t) %>% 
  mutate(share = 100 * forecast * flh * eff * 1e-3 / fe.eu) %>% 
...
data.stat.conv <- data.results %>% 
  ...
  drop_na(forecast) %>%
  filter(year == t) %>% 
  mutate(share = 100 * forecast * flh * eff * 1e-3 / fe.glob) %>% 
  mutate(type = "Conventional")
...
print(p)
ggsave(paste0(path.output, "EXT_FIG01_wind_solar.png"),
       width = 18, height = 16, bg = "white", units = "cm")
ggsave(paste0(path.output, "EXT_FIG01_wind_solar.svg"),
       width = 18, height = 16, bg = "white", units = "cm")
...
print(p)
ggsave(paste0(path.output, "EXT_FIG02_growth_different_slice_length.png"),
       width = 16, height = 8, units = "cm", bg = "white")
ggsave(paste0(path.output, "EXT_FIG02_growth_different_slice_length.svg"),
       width = 16, height = 8, units = "cm", bg = "white")

```

## EXT FIG 3: Conventional growth sensitivity analysis of anticipation
...
print(p)
ggsave(paste0(path.output, "EXT_FIG03_sensitivity_anticipation_conventional.png"),
       width = 18, height = 18, units = "cm", bg = "white")
ggsave(paste0(path.output, "EXT_FIG03_sensitivity_anticipation_conventional.svg"),
       width = 18, height = 18, units = "cm", bg = "white")

```

## EXT FIG 4: Unconventional growth sensitivity analysis of anticipation
...
print(p)
ggsave(paste0(path.output, "EXT_FIG04_sensitivity_anticipation_unconventional.png"),
       width = 18, height = 18, units = "cm", bg = "white")
ggsave(paste0(path.output, "EXT_FIG04_sensitivity_anticipation_unconventional.svg"),
       width = 18, height = 18, units = "cm", bg = "white")

```

## EXT FIG 5: Electrolysis capacity distribution for additional years
...
print(p)
ggsave(paste0(path.output, "EXT_FIG05_capacity_distributions_2025-2050.png"),
       width = 18, height = 14, units = "cm", bg = "white")
ggsave(paste0(path.output, "EXT_FIG05_capacity_distributions_2025-2050.svg"),
       width = 18, height = 14, units = "cm", bg = "white")
